...
interface Chain<T> {
  ...
  /** 任务完成后执行 */
  completed: (
    onCompleted: (controller: AbortChainController) => void | PromiseLike<void>,
  ) => Chain<T>;
  /** 任务中断后执行 */
...
export const createAbortChain = <T>(initial?: T): Chain<T> => {
  ...
  /** 任务链 */
  const chain: Array<
    (value: T, controller: AbortChainController) => TResult | PromiseLike<TResult>
  > = [];
  ...
  let onCapture:
    | ((reason: any, controller: AbortChainController) => TResult | PromiseLike<TResult>)
    | undefined;
