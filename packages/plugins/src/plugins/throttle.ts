...
/** Error raised when a throttled request uses the throwing give-up rule. */
export class ThrottleError extends Error {
  type: string = "throttle";
}

/** Behavior applied when a duplicate request is throttled. */
...
/** Options for rejecting or silencing duplicate requests within a delay window. */
...
/**
 * Applies the configured give-up rule to duplicate requests within the throttle window.
 */
export const throttle = (options: IThrottleOptions = {}): IPlugin => {
  ...
  return {
    ...
    lifecycle: {
        ...
        handler: async (config, { origin, shared }, { abort, abortError, slient }) => {
          ...
          if (cache[hash]) {
            ...
            let throttleErrorMessage: IThrottleOptions["throttleErrorMessage"] =
              (config.throttle as IThrottleOptions)?.throttleErrorMessage ??
              options.throttleErrorMessage;
      ...
      completed: {
        runWhen: (hookOptions) => {
          return runWhen(undefined, hookOptions);
        },
