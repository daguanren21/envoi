...
/** Options for redirecting selected requests to a mock server. */
...
/**
 * Rewrites matching request URLs so they are dispatched to a configured mock server.
 *
    ...
    lifecycle: {
      preRequestTransform: {
        runWhen(_, { origin }) {
          ...
          const mockOptions = origin.mock ?? options.mock;
          if (typeof mockOptions === "object") {
            return !!mockOptions.mock;
          } else {
            return !!mockOptions;
          }
        },
